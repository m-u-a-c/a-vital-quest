...
public class First_AI : MonoBehaviour {
	...
	void EMovement()
	{
		...
		if(grounded && player.transform.position.y > transform.position.y + 0.5f)
		{
			if ((groundAroundLB && !groundAroundL2T) || (groundAroundRB && !groundAroundR2T))
			{
				rigidbody2D.AddForce(new Vector2(0, (jumpForce/175)));
			}
			if ((groundAroundLB && !groundAroundLT) || (groundAroundRB && !groundAroundRT))
			{
				rigidbody2D.AddForce(new Vector2(0, (jumpForce/175)));
			}
		}
